...
(function ($, Drupal) {
  ...
  Drupal.behaviors.toggleMenu = {
    attach: function attach(context, settings) {
      $('#mobnav-btn', context).on('click', function (e) {
        $(this).toggleClass('active');
        $('.hamburger__container').toggleClass('active');
        $('#block-mainnavigation').toggleClass('active');
        $('html').toggleClass('mobile-active');
        $('#block-exposedformacquia-searchpage').removeClass('active');
      });
      $('#block-mainnavigation > ul > .main__item', context).on('mouseenter focusin', function (e) {
        if ($(window).width() < 960) {
          $(this).addClass('active');
        }
      });
      var isMobile = true,
          topLevelNavLink = $('.main > li > a', context),
          hasSubMenuLink = $('.main > li .is-expanded > a', context),
          mainSubmenuLastLink = $('.main__submenu > li:last-child > a', context);
      var toggleSearch = $('#toggle-search', context);
      $('#mobnav-btn', context).on('keyup', function (event) {
        if (event.key == "Enter") {
          $('#main-menu > li:first-child > a').focus();
        }
      });
      $(document).on('keyup', function (event) {
        var focusedEl = $(':focus', context);

        if (event.key == 'Escape' && isMobile) {
          // Esc key
          $('#mobnav-btn', context).removeClass('active');
          $('.hamburger__container', context).removeClass('active');
          $('#block-mainnavigation', context).removeClass('active');
          $('html').removeClass('mobile-active');
        } else if (event.which == 40 && !isMobile && topLevelNavLink.is(':focus')) {
          $(':focus').parent().toggleClass('active');
        } else if (event.which == 32 && !isMobile && topLevelNavLink.is(':focus')) {
          $(':focus').parent().toggleClass('active');
        } else if (event.which == 9 && !isMobile && topLevelNavLink.is(':focus')) {
          // tab
          if (!$(':focus').parent().hasClass('active')) {
            // if tabbing to top level that does not have active class close other open menu
            topLevelNavLink.parent().removeClass('active');
          }
        }

        if (focusedEl.attr('id') == 'toggle-search' || focusedEl.attr('class') == 'branding__supported-by' || focusedEl.attr('class') == 'branding__logo') {
          topLevelNavLink.parent().removeClass('active');
        }
      });
      $('#main-menu > li:last-child li:last-child a').on('keydown', function (e) {
        if (isMobile) {
      ...
      $('#main-menu > li:first-child > a').on('keydown', function (e) {
        if (isMobile) {
          var isTabPressed = e.key === 'Tab' || e.keyCode === 9;
          ...
          if (e.shiftKey)
            /* shift + tab */
            {
              $('#main-menu > li:last-child li:last-child a').focus();
              e.preventDefault();
            } else
            /* tab */
            {
      ...
      }); // prevent default if using down arrow or spacebar when top level nav item has focus

      topLevelNavLink.on('keydown', function (e) {
        if (e.which == 32 || e.which == 40 && !isMobile) {
          e.preventDefault();
        }
      }).on('keyup', function (e) {
        if (e.keyCode == 9) {
      ...
      hasSubMenuLink.on('keydown', function (e) {
        if (e.which == 32 || e.which == 40 && !isMobile) {
          e.preventDefault();
        }
      }).on('keyup', function (e) {
        if (e.which == 32 && !isMobile) {
          // Space Bar
          $(this).parent().toggleClass('active');
        } // close if going back up
      ...
      }); // last item in each main__submenu

      mainSubmenuLastLink.on('keydown', function (e) {
        if (e.which == 9 && !isMobile) {
          // Tab Key
          // allow tabbing into sub if it's toggled open
          if (!$(this).parent().hasClass('active') && !e.shiftKey) {
            // go up one level and remove active class
      ...
      enquire.register("screen and (min-width:960px)", {
        match: function match() {
          isMobile = false;
          $('.main li', context).removeClass('active');
        },
  ...
  Drupal.behaviors.toggleSubMenus = {
    attach: function attach(context, settings) {
      $('.menu__sub-arrow', context).on('click keypress', function (e) {
        var subArrow = $(this, context),
            buttonState = subArrow.attr('aria-expanded'),
            target = subArrow.siblings('.menu__sub');

        if (buttonState === 'false') {
          target.toggleClass('open');
          subArrow.attr('aria-expanded', 'true');
        } else {
          target.toggleClass('open');
          subArrow.attr('aria-expanded', 'false');
        }
      });
      $('.menu__back', context).on('click keypress', function (e) {
        var backArrow = $(this, context),
            target = backArrow.parents('.menu__sub'),
            button = target.prev('.menu__sub-arrow');
  ...
  Drupal.behaviors.toggleSearch = {
    attach: function attach(context, settings) {
      var form = $('#block-exposedformacquia-searchpage', context),
          html = $('html', context),
          toggleSearch = $('#toggle-search', context),
          searchForm = $('#block-exposedformacquia-searchpage'),
      ...
      toggleSearch.on('click', function (e) {
        ...
        if (form.hasClass('active')) {
          setTimeout(setSearchFocus, 500);
        }

        if (html.hasClass('mobile-active')) {
          $('#mobnav-btn').removeClass('active');
      ...
      enquire.register("screen and (min-width:960px)", {
        match: function match() {
          toggleSearch.appendTo('#main-menu').wrap('<li id="search-container" class="search-container"></li>');
          searchForm.appendTo('#search-container');
        },
        unmatch: function unmatch() {
          toggleSearch.insertBefore('#block-useraccountmenu');
          searchForm.insertAfter(mainNav);
  ...
  Drupal.behaviors.smoothScroll = {
    attach: function attach(context, settings) {
      // modify skip link href on home page due to layout
      var homeSkipToMain = $('.path-frontpage .skip-link', context);

      if (homeSkipToMain.length > 0) {
        homeSkipToMain.attr('href', '#main-content');
      } // For smooth scrolling links


      $('a[href*="#"]:not([href="#"])', context).click(function () {
        if (location.pathname.replace(/^\//, '') == this.pathname.replace(/^\//, '') || location.hostname == this.hostname) {
          var target = $(this.hash, context);
  ...
  Drupal.behaviors.subscribeTaxonomyFloat = {
    attach: function attach(context, settings) {
      var subscribeFloatIFrame = $('.path-taxonomy .view-header #dyfo', context),
          subscribeFloatView = $('.path-taxonomy .view-header .field--type-text-with-summary', context);

      if (subscribeFloatIFrame.length) {
        subscribeFloatView.prepend('<button id="subscribe-float-close" class="subscribe-float__close"><span>x</span></button>');

        if (sessionStorage.getItem('widget-subscribe-float') == 'removed') {
          subscribeFloatView.css('display', 'none');
        }
        ...
        var floatingCtaTop = new Waypoint({
          ...
          handler: function handler(direction) {
            if (direction == 'down') {
              subscribeFloatView.removeClass('fade');
              subscribeFloatView.css('display', 'block');
              subscribeFloatView.css('z-index', 1);
            } else {
              subscribeFloatView.addClass('fade');
              subscribeFloatView.css('z-index', -1);
            }
          },
          offset: '10%'
        ...
        var floatingCtaBottom = new Waypoint({
          element: document.getElementById('page-footer', context),
          handler: function handler(direction) {
            if (direction == 'down') {
              subscribeFloatView.addClass('fade');
              subscribeFloatView.css('z-index', -1);
        ...
        $('#subscribe-float-close').on('click', function (elem) {
          $(this).parent().hide();
...
(function ($, Drupal) {
  ...
  Drupal.behaviors.subscribeFloat = {
    attach: function attach(context, settings) {
      var subscribeFloatWidget = $('#block-subscribetoourweeklynewsletterflyin', context),
